...
bis<-bfull %>% select(dx_hypertension, dx_kidney, hiv_test_result, bmi, bp_dia, tco2, spo2, blood, proteinuria, RI, kidneysize, qSOFA, AKI, age, sex, days_unwell, cl, hb, proteinuria, losscmd, hydronephrosis, inc_echogenicity, prior_mal, prior_mal,dx_diabetes, bun, an_gap, k, crea, died, CKD, f_time)
...
```{r}
set.seed(123)
fit1<-glm(AKI ~ RI,data = data, family=binomial(link='logit'))
fit2<-logbin(AKI ~ RI, data = data)
...
FINAL FULL MODEL - Figure 1c in the supplement
...
All you have to do is to resample your dataset with replacement and calculate the OR for each resampled dataset, then summarise the resulting empirical distribution of ORs 
 ...
 Start with the unadjusted model (age) complete case analysis using dataframe called bis
...
for(b in 1:B){
  ...
  adj_model<-lrm(AKI ~ rcs(age, 3) + hiv_test_result + qSOFA + bp_dia, data = datBS)
   # predict the odds for model fitted to bootstrapped data(  ## add code here to estimate the OR and save the OR for each bootstrapped dataset in orBS[b])
   pre<-Predict(adj_model,age=18:90, qSOFA=2, hiv_test_result=1, bp_dia=75,fun = plogis)
#pre contains probabilities - need to get the odds   
...
Adjusted model - using imputed dataset called data
...
for(b in 1:B){
  #draw a boostrap sample 
   datBS<-data[sample(1:nrow(data),size=nrow(data),replace=TRUE),]
  #fit base_model to bootstrapped data
  adj_model<-lrm(AKI ~ rcs(age, 3) + hiv_test_result + qSOFA + bp_dia, data = datBS)
   # predict the odds for model fitted to bootstrapped data(  ## add code here to estimate the OR and save the OR for each bootstrapped dataset in orBS[b])
   pre<-Predict(adj_model,age=18:90, qSOFA=2, hiv_test_result=1, bp_dia=75,fun = plogis)
...
```
 ...
 Unadjusted imputed dataset called data
...
```{r}
foresttab<-structure(list(mean = c(1.7,4.53,14.17,4.87,3.1),
               lower = c(1.13,2.24,4.38,1.38,1.09),
               upper = c(2.53,14.66,136.57,17.13,8.84)),
          .Names = c("mean", "lower", "upper"),
          row.names = c("50 years","60 years","70 years","HIV+","qSOFA"),
          class = "data.frame")

tabletext<-cbind(c("50 years","60 years","70 years","HIV+","qSOFA"),
                 c("1.70","4.53", "14.17", "4.87", "3.10"))
...
foresttab %>%
  forestplot(labeltext = tabletext,
             ...
             xlog = FALSE,
             zero =1,
             xlab= "Adjusted OR with 95% confidence interval",
...
```{r}
set.seed(123)
res.cox1<-coxph(formula = Surv(f_time, died) ~ age + hiv_test_result + strata(AKI) + qSOFA, data = bis) 
summary(res.cox1)
