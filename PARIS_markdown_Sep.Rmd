...
write_csv(raw_data,here("raw_data.csv"))
```
...
Estimating baseline creatinine, given this is a young population with low BMI - using eGFR of 75 is likely to under detect AKI 
assume eGFR of 100 would be more accurate but lets look at both methods


```{r}
b<-bfull %>% select(crea, sex, age, weight, cr_48h, cr_3m, blood, proteinuria) %>%
...
newAKI<-b %>%
  mutate(eGFR = MDRD4(creatinine, sex, age, ethnicity, method = "other"),
   ...
   mutate(PKD=if_else(AKI=="yes" & cr_3m >=1.5*esCr, "yes","no"),
          CKD_MDRD=if_else(eGFR<60 & eGFR_3m<60, "yes","no"),
          CKD_EPI=if_else(eGFR<60 & CKDEPI_3m<60, "yes","no"),
          CKD=if_else(CKD_EPI=="yes" |CKD_MDRD=="yes", "yes", "no"),
          AKD=if_else(eGFR<60 | AKI=="yes" |blood=="Yes"|cr_48h>=1.5*creatinine|
                        proteinuria =="Positive", "yes","no"))

newAKI<-newAKI %>% replace_na(list(AKD ="no"))

#get lowest Cr

lowcr <- bfull %>% 
  select(pid, cr_0h, cr_48h, cr_3m) %>%
  rowwise() %>%
  summarise(pid = pid, nadir = min(cr_0h, cr_48h, cr_3m, na.rm = TRUE))

bfull<-bfull %>% left_join(lowcr, by="pid") %>%
mutate(minAKI0h=case_when(cr_0h>=1.5*nadir & cr_0h<2*nadir ~"1",
                                cr_0h>=2*nadir & cr_0h<3*nadir ~ "2",
                                cr_0h>=3*nadir ~ "3", 
                                cr_0h<1.5*nadir ~ "no"))

xy<-bfull %>% mutate(nadAKI=if_else(cr_0h>=1.5*nadir, true = "yes", false="no"))


bfull<-bfull %>% mutate(AKI_baselinenadir = if_else(minAKI0h == "1" | minAKI0h == "2" | minAKI0h == "3",
             true = "yes", false = "no"))

newAKI<-newAKI %>% select(eGFR,eGFR_3m,CrCl, AKI, stage, AKI75, CKD_MDRD,CKD_EPI, CKD, estAKI, PKD, recovered, incidentAKI, AKD)
...
AKDs<-bfull %>% select(eGFR, cr_0h, cr_48h, cr_3m, eGFR_3m, AKI, blood, proteinuria)

bfull<- bfull %>% mutate(AKD_3m = if_else(eGFR>60 & eGFR_3m<=eGFR-(0.35*eGFR) | eGFR>60 & cr_3m>=1.5*cr_0h, "yes","no"))

#remove CKDs from  = n = 7

bfull %>% filter(AKD=="yes" & CKD_MDRD=="yes" | CKD_MDRD=="yes")

#identify new from old CKD - cant technically identify new CKD as would have to have eGFR>60 at baseline and then <60 at 3 months so havent proven the 3 months

bfull %>% filter(eGFR>60 & CKD_MDRD=="yes"|CKD_EPI=="yes")

bfull %>% filter(eGFR<60 & eGFR_3m<60)
```
...
```{r}
outcomes<-bfull %>% select(cr_0h, cr_48h, cr_3m, recovered, alive)

sum(is.na(outcomes$cr_48h))
sum(is.na(outcomes$cr_3m))
```


Get the FAS equation Cr
first split group into <40 years and >40 years
```{r}

a<-bfull %>% filter(age<40 & age>=20 & sex=="female") %>% 
  select(pid, age, sex) %>%
  mutate(q=62, eGFR=100, SCr=1.073*q)

b<-bfull %>% filter(age<40 & age>=20 & sex=="male") %>% 
  select(pid, age, sex) %>%
  mutate(q=80, eGFR=100, SCr=1.073*q)

c<-bfull %>% filter(age<20 & age<19 & sex=="female") %>% 
  select(pid,age, sex) %>%
  mutate(q=61, eGFR=100, SCr=1.073*q)

d<-bfull %>% filter(age<20 & age>19 & sex=="female") %>% 
  select(pid,age, sex) %>%
  mutate(q=62, eGFR=100, SCr=1.073*q)

e<-bfull %>% filter(age<20 & sex=="male") %>% 
  select(pid,age, sex) %>%
  mutate(q=78, eGFR=100, SCr=1.073*q)

f<-bfull %>% filter(age>40 & sex=="male") %>% 
  select(pid,age, sex) %>%
  mutate(q=80, eGFR=100, SCr=0.931966449*q*(0.988^(age-40)))

g<-bfull %>% filter(age>40 & sex=="female") %>% 
  select(pid,age, sex) %>%
  mutate(q=62, eGFR=100, SCr=0.931966449*q*(0.988^(age-40)))

SCr<-a %>% bind_rows(b, c, d, e, f, g) %>%
  select(pid, SCr) %>%
  mutate(SCr=SCr*0.0113)

bfull<-bfull %>% left_join(SCr, by = "pid")


#now classify AKI status

bfull<-bfull %>% mutate(FASAKI=if_else(cr_0h>=1.5*SCr, true="yes", false="no"))


```

```{r}
recover<-bfull %>% filter(AKI=="yes") %>%
  select(cr_0h, cr_48h, cr_3m, stage, CKD_EPI, CKD_MDRD, alive) %>%
  mutate(recovering = if_else(cr_48h<=cr_0h-0.3 | cr_3m<1.3, true = "yes", false = "no"))


recoverAKD<-bfull %>% filter(AKD=="yes") %>%
  select(cr_0h, cr_48h, cr_3m) %>%
  mutate(recovering = if_else(cr_48h<=cr_0h-0.3 | cr_3m<1.3, true = "yes", false = "no"))


recover %>% filter(recovering=="yes")
recoverAKD %>% filter(recovering=="yes")
```

Looking at CKD

```{r}
bfull %>% filter(CKD_MDRD=="yes" | CKD_EPI=="yes") -> CKDchar

CKDchar %>% filter(hiv_test_result=="positive")
```


```{r}
died<-bfull %>% mutate(died=if_else(alive=="1", t="0","1"))

died %>% select(AKI, FASAKI, AKI75, AKI_baselinenadir, AKD, alive) %>%
  tbl_summary(by=alive) %>% add_n() %>% add_p

died %>% select(AKI, died) %>%
  tbl_summary(by=AKI) %>% add_n() %>% add_p

died %>% select(FASAKI, died) %>%
  tbl_summary(by=FASAKI) %>% add_n() %>% add_p

died %>% select(AKI_baselinenadir, died) %>%
  tbl_summary(by=AKI_baselinenadir) %>% add_n() %>% add_p

died %>% select(AKD, died) %>%
  tbl_summary(by=AKD) %>% add_n() %>% add_p
```
...
```{r}
bfull %>% select(hiv_test_result, AKI) %>%
  tbl_summary(by=AKI) %>% add_n() %>% add_p
```
...
```{r}
bfull %>% select(hiv_test_result, CKD_EPI) %>%
  tbl_summary(by=CKD_EPI) %>% add_n() %>% add_p
```


```{r}
bfull %>% select(hiv_test_result, CKD_MDRD) %>%
  tbl_summary(by=CKD_MDRD) %>% add_n() %>% add_p
```
...
variable == "FASAKI" ~ "Acute Kidney Injury at presentation FAS equation n/N (%)",
variable == "AKI_baselinenadir" ~ "Acute Kidney Injury using nadir as baseline n/N (%)",
variable == "AKD" ~ "Acute Kidney Disease at presentation n/N (%)",
variable == "AKD_3m" ~ "Acute Kidney Disease at 3m n/N (%)",
...
variable == "recovered" ~ "AKI recovery by 48h**",
 variable == "CKD_MDRD" ~  "Chronic kidney disease MDRD (eGFR < 60 ml/min) at 3 months¶ ",
variable == "CKD_EPI" ~  "Chronic kidney disease EPI (eGFR < 60 ml/min) at 3 months¶ ",
...
cattab <-c %>%
  ...
  rename(p.value = V4, variable = name) %>% 
  p_round(digits=2) %>%
...
```{r}
df.sum.tbl %>% 
  select(age, bmi, urineprot, kidneysize, bp_sys, bp_dia, hr, temp, spo2, rr, days_unwell, qSOFA, RI, hb, bun, cl, na, k, tco2, crea, an_gap,
         eGFR, CrCl, AKI) %>%
  group_by(AKI) %>%
...
calulated from estimated creatinine and baseline Cr
(because not much incident AKI at 48h)
...
```
Characteristics of those with CKD

```{r}
bfull %>% filter(CKD_MDRD=="yes" | CKD_EPI=="yes") %>%
  select(AKI, AKI_baselinenadir, AKI75, AKD, eGFR) -> CKDs

prop.table(table(CKDs$AKI))
prop.table(table(CKDs$AKD))
prop.table(table(CKDs$AKI_baselinenadir))
prop.table(table(CKDs$AKI75))
```
...
bis<-bfull %>% select(dx_hypertension, dx_kidney, hiv_test_result, bmi, bp_dia, tco2, spo2, blood, proteinuria, RI, kidneysize, qSOFA, AKI, FASAKI, AKD, AKI_baselinenadir, age, sex, days_unwell, cl, hb, proteinuria, losscmd, hydronephrosis, inc_echogenicity, prior_mal, prior_mal,dx_diabetes, bun, an_gap, k, crea, died, CKD_EPI, CKD_MDRD, f_time)


bis <- bis %>% 
  mutate(hiv_test_result = if_else(hiv_test_result=="positive",1,0),
         AKI=if_else(AKI=="yes",1,0),
         FASAKI=if_else(FASAKI=="yes",1,0),
         CKD=if_else(CKD_EPI=="yes",1,0),
         AKD=if_else(AKD=="yes",1,0),
        AKI_baselinenadir=if_else(AKI_baselinenadir=="yes",1,0),
         f_time=as.numeric(f_time))

```


Explore the RI association

```{r}

bfull %>%
  ggplot(aes(x=AKI, y=RI, color=AKI)) + 
  geom_boxplot() + 
...
Present a histogram of presenting Creatinine - needs to be on the log scale

```{r}
hist(bfull$crea)

ggplot(bfull, aes(x = crea)) + geom_histogram() + scale_x_log10() + labs(title="Creatinine values on admission", x="Creatinine (mg/dL)", y="")
```

Plot age as a spline term unadjusted model
```{r}
...
Simply bootstap to get the 95% CI

For bootstrapping:
All you have to do is to resample your dataset with replacement and calculate the OR for each resampled dataset, then summarise the resulting empirical distribution of ORs 

 Start with the unadjusted model (age) complete case analysis using dataframe called bis - incorporating feedback from reviewer 2 - having age categories spanning 20 years - AKI
...
for(b in 1:B){
  #draw a boostrap sample from bis
   datBS<-bis[sample(1:nrow(bis),size=nrow(bis),replace=TRUE),]
  #fit base_model to bootstrapped data
   base_model<-lrm(AKI ~ rcs(age, 3), data = datBS)
   # predict the odds for model fitted to bootstrapped data(  ## add code here to estimate the OR and save the OR for each bootstrapped dataset in orBS[b])
   pre<-Predict(base_model,age=18:90,fun = plogis)
#pre contains probabilities - need to get the odds   
orBS[b]<-((pre[pre$age==20,2])/(1-(pre[pre$age==20,2]))) / ((pre[pre$age==40,2])/(1-(pre[pre$age==40,2])))

}
   
or95CI=quantile(probs=c(0.025,0.975),orBS)
print(orBS[b])
print(or95CI)
...
for(b in 1:B){
  ...
  #fit base_model to bootstrapped data
   ...
   pre<-Predict(base_model,age=18:90,fun = plogis)
#pre contains probabilities - need to get the odds   
orBS[b]<-((pre[pre$age==30,2])/(1-(pre[pre$age==30,2]))) / ((pre[pre$age==40,2])/(1-(pre[pre$age==40,2])))
...
for(b in 1:B){
  ...
  #fit base_model to bootstrapped data
   ...
   pre<-Predict(base_model,age=18:90,fun = plogis)
#pre contains probabilities - need to get the odds   
orBS[b]<-((pre[pre$age==50,2])/(1-(pre[pre$age==50,2]))) / ((pre[pre$age==40,2])/(1-(pre[pre$age==40,2])))
...
for(b in 1:B){
  ...
  #fit base_model to bootstrapped data
   ...
   pre<-Predict(base_model,age=18:90,fun = plogis)
#pre contains probabilities - need to get the odds   
orBS[b]<-((pre[pre$age==60,2])/(1-(pre[pre$age==60,2]))) / ((pre[pre$age==40,2])/(1-(pre[pre$age==40,2])))
...
for(b in 1:B){
  ...
  #fit base_model to bootstrapped data
   ...
   pre<-Predict(base_model,age=18:90,fun = plogis)
#pre contains probabilities - need to get the odds   
orBS[b]<-((pre[pre$age==70,2])/(1-(pre[pre$age==70,2]))) / ((pre[pre$age==40,2])/(1-(pre[pre$age==40,2])))
...
for(b in 1:B){
  ...
  #fit base_model to bootstrapped data
   ...
   pre<-Predict(base_model,age=18:90,fun = plogis)
#pre contains probabilities - need to get the odds   
orBS[b]<-((pre[pre$age==80,2])/(1-(pre[pre$age==80,2]))) / ((pre[pre$age==40,2])/(1-(pre[pre$age==40,2])))
...
Adjusted model - using complete case analysis dataset called bis
...
for(b in 1:B){
  ...
  adj_model<-lrm(AKI ~ rcs(age, 3) + hiv_test_result, data = datBS)
   # predict the odds for model fitted to bootstrapped data(  ## add code here to estimate the OR and save the OR for each bootstrapped dataset in orBS[b])
   pre<-Predict(adj_model,age=18:90, hiv_test_result=1,fun = plogis)
#pre contains probabilities - need to get the odds   
orBS[b]<-((pre[pre$age==20,5])/(1-(pre[pre$age==20,5]))) / ((pre[pre$age==40,5])/(1-(pre[pre$age==40,5])))
...
for(b in 1:B){
  ...
  adj_model<-lrm(AKI ~ rcs(age, 3) + hiv_test_result, data = datBS)
   ...
   pre<-Predict(adj_model,age=18:90, hiv_test_result=1,fun = plogis)
#pre contains probabilities - need to get the odds   
orBS[b]<-((pre[pre$age==30,5])/(1-(pre[pre$age==30,5]))) / ((pre[pre$age==40,5])/(1-(pre[pre$age==40,5])))
...
for(b in 1:B){
  ...
  adj_model<-lrm(AKI ~ rcs(age, 3) + hiv_test_result, data = datBS)
   ...
   pre<-Predict(adj_model,age=18:90, hiv_test_result=1,fun = plogis)
#pre contains probabilities - need to get the odds   
orBS[b]<-((pre[pre$age==50,5])/(1-(pre[pre$age==50,5]))) / ((pre[pre$age==40,5])/(1-(pre[pre$age==40,5])))
...
for(b in 1:B){
  ...
  adj_model<-lrm(AKI ~ rcs(age, 3) + hiv_test_result, data = datBS)
   ...
   pre<-Predict(adj_model,age=18:90,hiv_test_result=1,fun = plogis)
#pre contains probabilities - need to get the odds   
orBS[b]<-((pre[pre$age==60,5])/(1-(pre[pre$age==60,5]))) / ((pre[pre$age==40,5])/(1-(pre[pre$age==40,5])))
...
for(b in 1:B){
  ...
  adj_model<-lrm(AKI ~ rcs(age, 3) + hiv_test_result, data = datBS)
   ...
   pre<-Predict(adj_model,age=18:90, hiv_test_result=1, fun = plogis)
#pre contains probabilities - need to get the odds   
orBS[b]<-((pre[pre$age==70,5])/(1-(pre[pre$age==70,5]))) / ((pre[pre$age==40,5])/(1-(pre[pre$age==40,5])))
...
for(b in 1:B){
  ...
  adj_model<-lrm(AKI ~ rcs(age, 3) + hiv_test_result, data = datBS)
   ...
   pre<-Predict(adj_model,age=18:90, hiv_test_result=1,fun = plogis)
#pre contains probabilities - need to get the odds   
orBS[b]<-((pre[pre$age==80,5])/(1-(pre[pre$age==80,5]))) / ((pre[pre$age==40,5])/(1-(pre[pre$age==40,5])))
...
Now - using FAS - equation for creatinine

Start with the unadjusted model (age) complete case analysis using dataframe called bis - incorporating feedback from reviewer 2 - having age categories spanning 20 years - -FAS - AKI
...
for(b in 1:B){
  #draw a boostrap sample from bis
   datBS<-bis[sample(1:nrow(bis),size=nrow(bis),replace=TRUE),]
  #fit base_model to bootstrapped data
   base_model<-lrm(FASAKI ~ rcs(age, 3), data = datBS)
   # predict the odds for model fitted to bootstrapped data(  ## add code here to estimate the OR and save the OR for each bootstrapped dataset in orBS[b])
   pre<-Predict(base_model,age=18:90,fun = plogis)
#pre contains probabilities - need to get the odds   
orBS[b]<-((pre[pre$age==20,2])/(1-(pre[pre$age==20,2]))) / ((pre[pre$age==40,2])/(1-(pre[pre$age==40,2])))
...
for(b in 1:B){
  #draw a boostrap sample from bis
   datBS<-bis[sample(1:nrow(bis),size=nrow(bis),replace=TRUE),]
  #fit model to bootstrapped data
  adj_model<-lrm(FASAKI ~ rcs(age, 3) + hiv_test_result, data = datBS)
   # predict the odds for model fitted to bootstrapped data(  ## add code here to estimate the OR and save the OR for each bootstrapped dataset in orBS[b])
   pre<-Predict(adj_model,age=18:90,hiv_test_result=1,fun = plogis)
#pre contains probabilities - need to get the odds   
orBS[b]<-((pre[pre$age==60,5])/(1-(pre[pre$age==60,5]))) / ((pre[pre$age==40,5])/(1-(pre[pre$age==40,5])))
...
probability AKI for different scenarios for the tables

```{r}
fullmodel<-lrm(AKI ~ rcs(age, 3) + hiv_test_result + bp_dia + qSOFA, data = bis)
pre<-Predict(fullmodel,age=20:80, bp_dia=80, qSOFA=3,hiv_test_result=1,fun = plogis)


```


Complete Cases Analysis - using unimputed dataset bis
...
```{r}
badgone<-bis %>% filter(!RI<0.5)
set.seed(123)
#fit1<-glm(AKI ~ RI, data = badgone, family=binomial(link='logit'))
fit2<-logbin(AKI ~ RI, data = badgone)
#tidy(fit1, exponentiate = TRUE, conf.int = TRUE) 
tidy(fit2, exponentiate = TRUE, conf.int = TRUE)
```
...
```{r}
set.seed(123)
fit1<-glm(AKI ~ rcs(age,3) + hiv_test_result + sex,data = bis, family=binomial(link='logit'))
tidy(fit1, exponentiate = TRUE, conf.int = TRUE) 
...
```{r}
set.seed(123)
fit1<-glm(AKI ~ rcs(age,3) + hiv_test_result + bmi,data = bis, family=binomial(link='logit'))
tidy(fit1, exponentiate = TRUE, conf.int = TRUE) 
```
```{r}
set.seed(123)
fit1<-glm(AKI ~ rcs(age,3) + hiv_test_result,data = bis, family=binomial(link='logit'))
tidy(fit1, exponentiate = TRUE, conf.int = TRUE) 
```

```{r}
set.seed(123)
fit1<-glm(AKI ~ rcs(age,3) + hiv_test_result + dx_hypertension,data = bis, family=binomial(link='logit'))
tidy(fit1, exponentiate = TRUE, conf.int = TRUE) 
```

```{r}
set.seed(123)
fit1<-glm(AKI ~ rcs(age,3) + hiv_test_result + blood,data = bis, family=binomial(link='logit'))
tidy(fit1, exponentiate = TRUE, conf.int = TRUE) 
```

```{r}
set.seed(123)
fit1<-glm(AKI ~ rcs(age,3) + hiv_test_result + proteinuria,data = bis, family=binomial(link='logit'))
tidy(fit1, exponentiate = TRUE, conf.int = TRUE) 
```

```{r}
set.seed(123)
fit1<-glm(AKI ~ rcs(age,3) + hiv_test_result + kidneysize,data = bis, family=binomial(link='logit'))
tidy(fit1, exponentiate = TRUE, conf.int = TRUE) 
```

```{r}
set.seed(123)
fit1<-glm(AKI ~ rcs(age,3) + hiv_test_result + inc_echogenicity,data = bis, family=binomial(link='logit'))
tidy(fit1, exponentiate = TRUE, conf.int = TRUE) 
```

```{r}
set.seed(123)
fit1<-glm(AKI ~ rcs(age,3) + hiv_test_result + losscmd,data = bis, family=binomial(link='logit'))
tidy(fit1, exponentiate = TRUE, conf.int = TRUE) 
```

```{r}
set.seed(123)
fit1<-glm(AKI ~ rcs(age,3) + hiv_test_result + qSOFA,data = bis, family=binomial(link='logit'))
tidy(fit1, exponentiate = TRUE, conf.int = TRUE) 
```

```{r}
set.seed(123)
fit1<-glm(AKI ~ rcs(age,3) + hiv_test_result + spo2,data = bis, family=binomial(link='logit'))
tidy(fit1, exponentiate = TRUE, conf.int = TRUE) 
```

```{r}
set.seed(123)
fit1<-glm(AKI ~ rcs(age,3) + hiv_test_result + bp_dia,data = bis, family=binomial(link='logit'))
tidy(fit1, exponentiate = TRUE, conf.int = TRUE) 
```

```{r}
set.seed(123)
fit1<-glm(AKI ~ rcs(age,3) + hiv_test_result + tco2,data = bis, family=binomial(link='logit'))
tidy(fit1, exponentiate = TRUE, conf.int = TRUE) 
```

```{r}
set.seed(123)
fit1<-glm(AKI ~ rcs(age,3) + hiv_test_result + RI,data = badgone, family=binomial(link='logit'))
tidy(fit1, exponentiate = TRUE, conf.int = TRUE) 
```
...
fit1<-glm(AKI ~ rcs(age,3) + hiv_test_result,data = subset(bis, !is.na(AKD)), family=binomial(link='logit'))
tidy(fit1, exponentiate = TRUE, conf.int = TRUE) 
...
Comparing different types of kidney disease at presentation
```{r}

data <- data.frame(bis$AKI,bis$died)
  
# Create a contingency table with the needed variables.           
data <- table(bis$AKI,bis$died) 
                  
print(data)

print(chisq.test(data))

CrossTable(bis$AKI, bis$died, prop.t=TRUE, prop.r=TRUE, prop.c=TRUE)
```

```{r}

data <- table(bis$FASAKI,bis$died) 
print(data)
print(chisq.test(data))
CrossTable(bis$FASAKI, bis$died, prop.t=TRUE, prop.r=TRUE, prop.c=TRUE)
```


```{r}

data <- table(bis$AKI_baselinenadir,bis$died) 
                  
print(data)

print(chisq.test(data))
CrossTable(bis$AKI_baselinenadir, bis$died, prop.t=TRUE, prop.r=TRUE, prop.c=TRUE)
```


```{r}
data <- data.frame(bis$AKD,bis$died)
  
# Create a contingency table with the needed variables.           
data <- table(bis$AKD,bis$died) 
                  
print(data)

print(chisq.test(data))

CrossTable(bis$AKD, bis$died, prop.t=TRUE, prop.r=TRUE, prop.c=TRUE)
```
```{r}
data <- data.frame(bis$AKI_baselinenadir,bis$died)
...
```{r}
ggsurvplot(
  survfit(Surv(f_time, died) ~ AKD,
          data = subset(bis, !is.na(AKD))), 
          conf.int = F, xlim = c(0, 90), 
          ylim = (c(0.6,1)), ggtheme = theme_bw(), 
          size = 0.5, 
          break.time.by = 30, 
...
```{r}
ggsurvplot(
  survfit(Surv(f_time, died) ~ AKI_baselinenadir,
          data = subset(bis, !is.na(AKI_baselinenadir))), 
          conf.int = F, xlim = c(0, 90), 
          ylim = (c(0.6,1)), ggtheme = theme_bw(), 
          size = 0.5, 
          break.time.by = 30, 
...
```{r}
ggsurvplot(
  survfit(Surv(f_time, died) ~ FASAKI,
          data = subset(bis, !is.na(FASAKI))), 
          conf.int = F, xlim = c(0, 90), 
          ylim = (c(0.6,1)), ggtheme = theme_bw(), 
          size = 0.5, 
          break.time.by = 30, 
...
```{r}
res.cox<-coxph(formula = Surv(f_time, died) ~ hiv_test_result + age + AKI, data = bis)
res.cox<-coxph(formula = Surv(f_time, died) ~ AKI, data = bis)
...
```{r}
set.seed(123)
res.cox1<-coxph(formula = Surv(f_time, died) ~ age + hiv_test_result + AKI + qSOFA, data = bis) 
summary(res.cox1)
